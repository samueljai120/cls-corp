# CLS Corp — Autonomous AI Company

> An AI-powered company that researches markets, designs products, writes code, generates media, and manages its own operations. Zero human intervention required.

## Architecture

```
Brain (Claude Sonnet) → Strategic Directives
    ├── Engineering   → Code generation + validation
    ├── Sales         → Prospect research + outreach
    ├── Marketing     → Content + social media
    ├── R&D           → Product research + innovation
    └── Finance       → Cost tracking + optimization
```
...
| Layer | Purpose |
|-------|---------|
| Quality Gate | Blocks AI slop, fabrications, emoji spam |
| Critical Eval Gateway | Ground truth verification (0 fake customers) |
| Multi-LLM Code Review | 4-agent parallel verification |
| Research Validator | Evidence-based, kills weak ideas |
| Brain Freshness | Prevents stale/duplicate directives |
...
- **LLM Chain**: Gemini Flash → OpenRouter DeepSeek V3.2 → Ollama qwen2.5-coder
- **Web Apps**: Next.js + Supabase + Stripe + Tailwind
- **Images**: HuggingFace FLUX (free)
- **Videos**: LTX-2 Turbo / Wan 2.2 (free)
...
~$0.05-0.50/day — designed for minimal spend, maximum output.

---
*Built by CLS Corp. Fully autonomous. No human in the loop.*
